...
import (
	...
	repositories "github.com/gipo355/its-battistar-be-go/internal/repository"
	s "github.com/gipo355/its-battistar-be-go/internal/server"
	"github.com/gipo355/its-battistar-be-go/pkg/requests"
	"github.com/gipo355/its-battistar-be-go/pkg/responses"
)
...
func (authHandler *AuthHandler) Login(c echo.Context) error {
	...
	if user.ID == 0 ||
		(bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(loginRequest.Password)) != nil) {
